...
# Run 'Before Experiment' code from get_conditions_file
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sanlab/Desktop/ASH/regulation_of_craving/ROC_lastrun.py',
...
# Run 'Begin Experiment' code from get_window_size
# Get window width and height
window_width = win.clientSize[0]
window_height = win.clientSize[1]
# Run 'Begin Experiment' code from get_conditions_file
...
conditions_file = 'conditions/ROC_' + participant + '_Session' + str(expInfo['session']) + '_Run' + str(run_number) + '.csv'
...
stim_rating = visual.Slider(win=win, name='stim_rating',
    startValue=None, size=(1.0, 0.025), pos=(0, -0.2), units=None,
...
while continueRoutine:
    ...
    if title_text.status == STARTED:
        ...
        if tThisFlipGlobal > title_text.tStartRefresh + 2.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'title_text.stopped')
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if regulate_look.status == NOT_STARTED and tThisFlip >= jitter_duration-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'regulate_look.started')
        ...
        if regulate_look.status == STARTED:
            ...
            if tThisFlipGlobal > regulate_look.tStartRefresh + 2-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'regulate_look.stopped')
        ...
        if rating_text.status == STARTED:
            ...
            if tThisFlipGlobal > rating_text.tStartRefresh + 4.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rating_text.stopped')
        ...
        if stim_rating.status == NOT_STARTED and tThisFlip >= jitter_duration+7-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'stim_rating.started')
        ...
        if stim_rating.status == STARTED:
            ...
            if tThisFlipGlobal > stim_rating.tStartRefresh + 4-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'stim_rating.stopped')
        ...
        if stim_keyboard.status == NOT_STARTED and tThisFlip >= jitter_duration+7-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'stim_keyboard.started')
        ...
        if stim_keyboard.status == STARTED:
            ...
            if tThisFlipGlobal > stim_keyboard.tStartRefresh + 4-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'stim_keyboard.stopped')
...
while continueRoutine:
    ...
    if end_key_resp.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > end_key_resp.tStartRefresh + end_text_duration-frameTolerance:
            # keep track of stop time/frame for later
